...
def select_action(
    ...
    """
    Overview:
        Select actions for 4 players from visit counts of the root node.
    Arguments:
        - visit_counts (:obj:`np.ndarray`): The visit counts of the root node, shape (4, 324).
        - temperature (:obj:`float`): The temperature used to adjust the sampling distribution.
        - deterministic (:obj:`bool`): Whether to enable deterministic mode in action selection.
    Returns:
        - actions (:obj:`np.ndarray`): The selected actions for 4 players.
        - visit_count_distribution_entropy (:obj:`np.ndarray`): The entropy of the visit count distribution for each player.
    """
